    ...
    def on_train_batch_end(
        ...
        if not trainer.sanity_checking:
            ...
            for name in self.layers:
                ...
                if gradient_wrt_ipt != []:
                    try:
                        gradient_wrt_ipt = grad(gradient_wrt_ipt,x,retain_graph=True)[0]
                        self.gradient[name].append(torch.mean(gradient_wrt_ipt.detach(),axis = (0,1)).cpu().numpy())
            ...
            if batch_idx == self.config.batch_size:
                ...
                for name in self.gradient:
                    ...
                    plt.title(f"Layer {[ k for k,v in trainer.model.named_modules()][name]}")
                    ax.set_axis_off()
